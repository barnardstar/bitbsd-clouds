...
def dobackup():
    ...
    if sure == 'y':
        ...
        dtime = datetime.datetime.strftime(datetime.datetime.now(), '%H:%M:%S')
        print(dtime+': starting cln')
        print(sshcmd('', default_vps['ssh_port'], 'su - lightning -c "/usr/local/bin/lightningd --daemon > /dev/null &"'))
        secs = time.time() - start
        dtime = datetime.datetime.strftime(datetime.datetime.now(), '%H:%M:%S')
        print(dtime+': started.. node was offline for ' + str(int(secs)) + ' seconds')
        print(dtime+': downloading backup archive to local storage')
        sshdownload('/tmp/bck.tar', '/tmp/bck.tar')
        dtime = datetime.datetime.strftime(datetime.datetime.now(), '%H:%M:%S')
        print(dtime+': encrypting archive locally with GPG')
        with open('/tmp/bck.tar', 'rb') as f:
            status = gpg.encrypt_file(
                f, recipients=['lnuser@bitclouds.sh'],
                output='/tmp/clightning.tar.gpg')
        dtime = datetime.datetime.strftime(datetime.datetime.now(), '%H:%M:%S')
        print(dtime+': uploading encrypted archive to node')
        ...
        print('\n ######     END OF LINKS    ######')
        dtime = datetime.datetime.strftime(datetime.datetime.now(), '%H:%M:%S')
        print(dtime+': clean up locally, on node & ipfs')
